	...
	<body>
		...
		<div id="show-store" class="content scaffold-show" role="main">
			...
			<ol class="property-list store">
				...
				<g:if test="${storeInstance?.albums}">
				<li class="fieldcontain">
					<span id="albums-label" class="property-label"><g:message code="store.albums.label" default="Albums" /></span>
					
						<g:each in="${storeInstance.albums}" var="a">
						<span class="property-value" aria-labelledby="albums-label"><g:link controller="album" action="show" id="${a.id}">${a?.title}</g:link></span>
						</g:each>
